...
class load_xrr():
    def __init__(self):
        self.data_directory = "K:/SYNCHROTRON/Murphy/2022-10_P08_11014659_giri_hoevelmann_petersdorf/raw_data/raw", #"K:/SYNCHROTRON/Murphy/2022-04_P08_11013326_giri_hoevelmann_petersdorf/raw_data/raw",
        ...
        self.roi = (14, 91, 60, 22), # 
        self.roi_offset = 30,
        self.monitor = "ion2",
        self.calculate_abs = None,           
            ...
            for n in range(n_points):
                ...
                if self.detector_orientation[0] == "horizontal":            
                    p_bg0 = img[self.roi[1]:(self.roi[1]+self.roi[3]),
                                (self.roi[0]+self.roi[2]+self.roi_offset[0]):(self.roi[0]+2*self.roi[2]+self.roi_offse[0])].sum()
                    p_bg1 = img[self.roi[1]:(self.roi[1]+self.roi[3]),
                                (self.roi[0]-self.roi[2]-self.roi_offset[0]):(self.roi[0]-self.roi_offset[0])].sum()            
                elif self.detector_orientation[0] == "vertical":    
                    p_bg0 = img[(self.roi[1]+self.roi[3]+self.roi_offset[0]):(self.roi[1]+2*self.roi[3]+self.roi_offset[0]),
                                (self.roi[0]):(self.roi[0]+self.roi[2])].sum()
                    p_bg1 = img[(self.roi[1]-self.roi[3]-self.roi_offset[0]):(self.roi[1]-self.roi_offset[0]),
                                (self.roi[0]):(self.roi[0]+self.roi[2])].sum()       
                ...
                s_intens.append(abs(p_intens))
                s_e_intens.append(abs(p_e_intens))
